...
#09/27/2017 Be careful! now it comes the RAWS winds, so there is no need to tranform the wind anymore
#----------------------------------------------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------
def IntepreteRAWS(csvfile,STATION,ID):
    #Open the downloaded csv for information
    with open(csvfile) as filt_csv:
        pread = pandas.read_csv(filt_csv,skiprows=6)
        #print pread.columns
    '''
    Index([u'Station_ID', u'Date_Time', u'snow_interval_set_1', u'air_temp_set_1',
       u'relative_humidity_set_1', u'wind_speed_set_1',
       u'wind_direction_set_1', u'wind_gust_set_1', u'volt_set_1',
       u'solar_radiation_set_1', u'precip_accum_set_1', u'water_temp_set_1',
       u'peak_wind_speed_set_1', u'fuel_temp_set_1', u'fuel_moisture_set_1',
...
##    #    df.loc[:,'precip_accum_set'] = df.loc[:,'precip_accum_five_minute_set_1']
    ...
    #No need to use the group function anymore for RAWS because there is only one record for each hour
##    df.loc[:,'Precipitation'] = df.loc[:,'precip_accum_set_1'].groupby(df['Date_Hour']).transform('max')
...
##    df.loc[:,'WindGust'] = df.loc[:,'wind_gust_set_1'].groupby(df['Date_Hour']).transform('max')
    df.loc[:,'Precipitation'] = df.loc[:,'precip_accum_set_1']
    df.loc[:,'SolarRadiation'] = df.loc[:,'solar_radiation_set_1']
    df.loc[:,'AirTemperature'] = df.loc[:,'air_temp_set_1']
    df.loc[:,'RelativeHumidity'] = df.loc[:,'relative_humidity_set_1']
    df.loc[:,'MaxAirTemperature'] = df.loc[:,'air_temp_set_1']
    df.loc[:,'MinAirTemperature'] = df.loc[:,'air_temp_set_1']
    df.loc[:,'WindSpeed'] = df.loc[:,'wind_speed_set_1']
    df.loc[:,'WindDirection'] = df.loc[:,'wind_direction_set_1']
    df.loc[:,'WindGust'] = df.loc[:,'wind_gust_set_1']

    print len(df)
    #print df.ix[:,{'solar_radiation_set_1','Date_Time'}]
    ...
    #some issues happended here, sometime there are missing hours! if the records is less than 12 hours, do not rprocess the records into fire weather file
    if len(df) < 24:
    ...
    #Add a column for the standard time zone
    ...
    #No need to process windspeed anymore
...
fileWF13 = os.path.join(WorkSpace, "tx-raws.fw13")
...
##ten_days_ago = datetime.datetime.utcnow() - relativedelta(days=10)
##delta = datetime.datetime.utcnow()-ten_days_ago
##backdays = delta.days
...
daytupleList = [('200809161900', '200809181859'),('200809171900', '200809191859')]
...
'''
411102	LINDEN	-94.357778	33.005	DENT2
417901	BIRD	-98.628889	30.2625	BDTT2
418902	MATADOR WMA	-100.3445804	34.1175387	MATT2

'''


#need to get the list of raws station name and list of parameter name for raws (different from mesowest).
#al
...
for STATION,ID in Stations.items():
    #define the fire weather file name
    fwnm = "%s_fw13_mesowest.fw13"%(STATION,)
    fileWF13 = os.path.join(WorkSpace, fwnm)
    for datetuple in daytupleList:
        ...
        # Catch the error type from the urllib.urlretrieve(URL,csvfile)
            ...
            continue
            #exit()
        IntepreteRAWS(csvfile,STATION,ID)
##    try:
