...
def IntepreteMesoNet(csvfile,STATION,ID):
    ...
    #get the end of hour solar radiation for SOW calculation
    df.loc[:,'EndHourSolarRadiation'] = df.loc[:,'solar_radiation_set_1'].groupby(df['Date_Hour']).tail(1)
...
## Process today's data
    ...
    SIXTYDAYSTR = SixtyDay.strftime("%Y%m%d")

StartTime = SIXTYDAYSTR + START_UTC
...
'''
/precip queries
Query for precipitation accumulation data:
http://api.mesowest.net/v2/stations/precip?stid=FREUT&start=201509240700&end=201509250000&token=demotoken
Query for a state's precipitation totals over a time period: http://api.mesowest.net/v2/stations/precip?state=mn&start=201401020000&end=201401042359&token=demotoken
'''

################This should be work!############
# url= https://api.mesowest.net/v2/stations/timeseries?stid=KCLL&vars=precip_accum_one_hour&start=201710010700&end=201712010000&token=demotoken&output=csv
# using one hour accumulation precipitation to get only precipitation
# Get the variable list https://api.synopticlabs.org/v2/variables?&token=994a7e628db34fc68503d44c447aaa6f&stids=KCLL
# Found out on the maps that GAIL (419102,419101) are 1.5 mile distance
#                            Quitaque and CapRoack are 1 mile distance
Stations = {'FADT2': 418903}
...
varibles_asos = 'precip_accum_one_hour'
varibles_raws = 'precip_accum_24_hour'
...
for STATION,ID in Stations.items():
    try:
        print STATION,ID
        api_string = base_url + query_type + '?' + 'stid=' + STATION + '&vars=' + varibles_raws + \
            '&start=' + StartTime + '&end=' + EndTime + \
            '&token=' + MyToken + units + csv_format
        print api_string
