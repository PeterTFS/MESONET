...
# Created:     04/05/2017
# Updated： 02/12/2017 adding 33 more station into WIMSTest 
...
##If (OBS_SOLAR < 25) SOW = 3 Catch observations during the nighttime.
    ...
    if CurrentHourPrecip <= 0.1: ##the current hour precipitaiton is 0 and use solar radiation for get the SOW
        ...
        #instead of using monthly solar radiation for each station, using an averaged monthly value (calculated based on the existing 4 stations) 
        MaxHourSR = SRtable.loc[month,"SR"]
...
def IntepreteMesoNet(csvfile,STATION,ID):
    ...
    df.loc[:,'SolarRadiation'] = df.loc[:,'solar_radiation_set_1'].groupby(df['Date_Hour']).transform('median')
    ...
    #need to do some tricks on getting the recently 10 minutes mean for the last record! 
    df.loc[:, 'WindSpeed'] = df.loc[:, 'wind_speed_set_1'].groupby(df['Date_Hour']).transform(Last10Min)
    df.loc[:, 'WindDirection'] = df.loc[:, 'wind_direction_set_1'].groupby(df['Date_Hour']).transform(Last10Min)
...
srtable = os.path.join(WorkSpace, 'SRTable_General.csv')
SRtable = pandas.read_csv(srtable, sep=',')
SRtable = SRtable.set_index(['Month'])
#print SRtable
#print SRtable.loc[6,"SR"]
...
# get the station dict from the predifined csv file
stationFile = os.path.join(WorkSpace, "AllMesoNetStations.csv")
stations_df = pandas.read_csv(stationFile, sep=',')
stations_df = stations_df[["SHEF ID","WIMS ID"]]
#transforming this two columns into a dict (key is the shef ID and value is WIMS ID)
Stations = dict(zip(stations_df.loc[:,"SHEF ID"],stations_df.loc[:,"WIMS ID"]))
#print Stations
